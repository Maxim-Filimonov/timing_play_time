defmodule TimingPlayTimeWeb.SettingsLiveTest do
  ...
  describe "Sign-in & devices" do
    test "anonymous User sees the link and sign-in affordances, no unverified state", %{conn: conn} do
      {:ok, _view, html} = live(conn, ~p"/settings")

      assert html =~ "Link an email"
      assert html =~ "Already linked? Sign in here"
      assert html =~ ~s(href="/auth/link")
