...
**Today's PT**:
The "today" component of Playtime: today's Timing-Derived Earned Total (local calendar day, per [ADR-0005](docs/adr/0005-local-timezone-for-day-boundaries.md)) minus today's Playtime Used. Resets to just today's activity every local calendar day, with no exceptions — Pushscroll Balance lives in Reserve instead (see below), since it has no day boundary of its own to reset by.

**Reserve**:
Play Minutes earned but not yet spent, that aren't scoped to today: prior-days' Timing-Derived Earned Total minus prior-days' Playtime Used, *plus* the current Pushscroll Balance (a Manual Sync value, above). Pushscroll Balance is grouped here rather than with Today's PT because both share the same shape — carried-over credit with no day boundary of its own — unlike Today's PT, which resets every midnight. Unclamped (can go negative). Shown as its own dashboard widget, separate from Today's PT, so today's activity and carried-over history stay visually distinguishable even though they combine into a single spendable Playtime number.
