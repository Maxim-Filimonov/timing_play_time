...
**Playtime**:
The dashboard's primary hero figure: Today's PT + Reserve. Unclamped — can go negative if usage has outpaced earnings. Distinct from Play Balance (above), which is the same total computed a different way and normally hidden. Also exactly equal to This Week's Earned minus This Week's Used plus Backlog Drawn plus Pushscroll Balance (see This Week) — a second, independent decomposition of the same number, shown alongside the hero figure specifically because it needs no explanation of flooring or causality to verify by eye, unlike Today's PT + Reserve.

**This Week**:
The Entry Expiry Window's raw (non-ledger) totals: every entry within the last 7 days' original earned minutes summed ("Earned"), and every Playtime Used record within the same window summed ("Used"), each with no consumption/FIFO logic applied. Shown per-Activity (gross earned only, since Playtime Used draws from one global pool, not a per-Activity one — see Playtime Used) and as a whole-User total alongside Today's. **Backlog Drawn** is the one figure here that does need the Entry Consumption Ledger: the portion of This Week's Used that was actually funded by entries *older* than the window (already off the books, not counted in This Week's Earned) rather than by this week's own earning — an expected occurrence, not an edge case, any week spending outpaces earning and Reserve still has older balance to draw on. `This Week's Earned − This Week's Used + Backlog Drawn + Pushscroll Balance == Playtime`, exactly, always — the Entry Consumption Ledger's `deficit` cancels out of this identity algebraically even though it's very much present inside Today's PT/Reserve's own math, which is what makes this pairing checkable by eye where Today's PT/Reserve isn't.

**Backlog Remaining**:
The flip side of Backlog Drawn (above): the sum of every out-of-window entry's `remaining` balance (Entry Consumption Ledger) — Play Minutes already earned, still unspent, but old enough to have aged out of the Entry Expiry Window and so no longer counted in Reserve's displayed figure. This is what a spend draws on, silently, once it exceeds everything Reserve visibly shows — the reason a User can log a large spend and see Reserve hold steady rather than go negative. Unbounded and Timing-entries-only, same as Backlog Drawn (Pushscroll Balance has no per-entry age to be "outside the window," so it's excluded). Never negative, since `remaining` floors at 0 per entry. Shown on the dashboard next to Backlog Drawn, always — even at 0.0 — on the same "predictable, not intermittent" principle as Spend Receipts.
