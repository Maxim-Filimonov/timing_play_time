...
**Reserve**:
Play Minutes earned but not yet spent, that aren't scoped to today: prior-days' User Displayed Total (itself already net of spending, via the Entry Consumption Ledger) *plus* the current Pushscroll Balance (a Manual Sync value, above). Pushscroll Balance is grouped here rather than with Today's PT because both share the same shape — carried-over credit with no day boundary of its own — unlike Today's PT, which resets every midnight. Can still go negative overall (if Pushscroll Balance itself is negative, or more was used today than earned today spills backward — see Today's PT), but never negative *purely from expiry*, since expiry only removes minutes already known to be unspent. Shown as its own dashboard widget, separate from Today's PT, so today's activity and carried-over history stay visually distinguishable even though they combine into a single spendable Playtime number.
